...
function routeLabels(result: RoutePlanResponse, index: number): string[] {
  ...
  const highestReliability = Math.max(...result.alternatives.map((item) => item.route_reliability));
  const shortestDuration = Math.min(...result.alternatives.map((item) => item.duration_seconds));
  ...
  if (result.alternatives.length === 0) {
    return (
      <section className="emptyResults" aria-live="polite">
        ...
        <p>Try a different departure time or a nearby stop.</p>
      </section>
    );
  }
  return (
    <section className="resultsSection" aria-labelledby="results-heading">
      ...
      <div className="routeList">
        {result.alternatives.map((alternative, index) => (
          <RouteCard
            ...
            selected={alternative.rank === selectedRank}
            onSelect={
              result.alternatives.length > 1 && onSelect
                ? () => onSelect(alternative.rank)
                : undefined
            }
          />
        ))}
      ...
      <details className="performanceDetails">
        <summary>
          <BarChart3 size={17} /> Performance details
        </summary>
        <dl>
          <div>
            <dt>Data loading</dt>
            <dd>{result.timing.data_loading_ms.toFixed(2)} ms</dd>
          </div>
          <div>
            <dt>Route search</dt>
            <dd>{result.timing.search_ms.toFixed(2)} ms</dd>
          </div>
          <div>
            <dt>Ranking</dt>
            <dd>{result.timing.ranking_ms.toFixed(2)} ms</dd>
          </div>
          <div>
            <dt>Total API time</dt>
            <dd>
              <Clock size={14} /> {result.timing.total_ms.toFixed(2)} ms
            </dd>
          </div>
        </dl>
      </details>
