...
score = 100 × (0.80 × route_reliability + 0.20 × speed_ratio)
```
...
The score is deterministic and clamped to `0–100`. Its 80/20
reliability/speed weighting lets a modestly slower but substantially more
reliable route win, while penalizing a large time increase for a tiny
reliability gain. Ties use reliability, arrival, transfers, and the stable
ordered trip/boarding/alighting identity.
