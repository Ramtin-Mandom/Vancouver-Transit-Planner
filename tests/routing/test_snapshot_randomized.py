...
def test_seeded_fifo_astar_dijkstra_differential(tmp_path):
    ...
    for number in range(80):
        ...
        connections.append(
            {
                "from_stop_id": f"S{source}",
                "to_stop_id": f"S{target}",
                "departure_seconds": departure,
                "arrival_seconds": departure
                + (target - source) * rng.randrange(180, 500),
                "trip_id": f"R{number}",
                "route_id": f"R{number}",
                "service_id": "ALL",
                "stop_sequence": 0,
        ...
        for _ in range(500):
            ...
            for algorithm in ("dijkstra", "astar"):
                ...
                if winners:
                    winner = winners[0]
                    validate_label_path(
                        snapshot.arrays, labels, winner, origin, destination, departure
                    )
                    validate_connection_path(
                        snapshot.arrays,
                        connection_path(labels, winner),
                        origin,
                        destination,
                        departure,
                        labels[winner].arrival,
                    )
    finally:
        snapshot.close()
