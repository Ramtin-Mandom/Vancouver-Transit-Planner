...
CONNECTIONS = [
    ...
    },
    {
        "from_stop_id": "B",
        "to_stop_id": "C",
        "departure_seconds": 29520,
        "arrival_seconds": 30600,
        "trip_id": "T2",
        "route_id": "R",
        "route_name": "10",
        "service_id": "S",
...
def test_incompatible_and_corrupt_snapshots(tmp_path):
    ...
    manifest.write_text(
        manifest.read_text().replace(
            f'"format_version": {snapshot_module.FORMAT_VERSION}',
            '"format_version": 999',
        )
    )
    with pytest.raises(SnapshotError, match="unsupported"):
        RoutingSnapshot(path)
...
def test_astar_and_dijkstra_agree_and_report_executed_algorithm(snapshot):
    ...
    assert (
        dijkstra.alternatives[0].itinerary.arrival_time
    ...
    assert astar.alternatives[0].itinerary.total_scheduled_travel_time == timedelta(
        minutes=35
    )
...
def test_validated_speed_bound_covers_every_spatial_edge(snapshot):
    ...
    for source, target, departure, arrival in zip(
        ...
        distance = haversine_meters(
            float(arrays["stop_lat"][source]),
            float(arrays["stop_lon"][source]),
            float(arrays["stop_lat"][target]),
            float(arrays["stop_lon"][target]),
        )
...
def test_gtfs_after_midnight_astar_matches_dijkstra(tmp_path):
    connections = [
        dict(CONNECTIONS[0], departure_seconds=87_000, arrival_seconds=87_600),
        dict(CONNECTIONS[1], departure_seconds=87_720, arrival_seconds=88_800),
    ]
    ...
    try:
        ...
        results = [
            planner.get_ranked_route_result(
                ...
                "C",
                date(2026, 8, 3),
                timedelta(seconds=86_900),
                algorithm=algorithm,
            )
            for algorithm in ("dijkstra", "astar")
        ]
        assert results[0].alternatives[0].itinerary.arrival_time == timedelta(
            seconds=88_800
        )
        assert (
            results[0].alternatives[0].itinerary.arrival_time
        ...
        connections.append(
            {
                "from_stop_id": "B",
                "to_stop_id": "C",
                "departure_seconds": 28_800,
                "arrival_seconds": 29_400,
                "trip_id": "UNREACHABLE",
        ...
        connections.append(
            {
                "from_stop_id": "A",
                "to_stop_id": "C",
                "departure_seconds": 28_900,
                "arrival_seconds": 33_000,
                "trip_id": "TOO-LATE",
                "route_id": "TOO-LATE",
                "service_id": "S",
                "stop_sequence": 1,
...
def test_snapshot_alternatives_obey_extra_window_and_preserve_fastest(tmp_path):
    ...
    try:
        ...
        assert [
            itinerary_identity(item.itinerary) for item in dijkstra.alternatives
...
def test_slower_equal_route_is_dominated(tmp_path):
    connections = [
        ...
        for arrival, trip, route in ((29_400, "FAST", "RF"), (30_000, "SLOW", "RS"))
    ]
    ...
    try:
        ...
        assert [item.itinerary.legs[0].trip_id for item in result.alternatives] == [
            "FAST"
        ]
    finally:
        loaded.close()
...
def test_adjacent_bay_suffix_detour_is_dominated(tmp_path):
    ...
    build_snapshot_from_rows(
        path, stops=stops, connections=connections, intra_station_transfer_seconds=120
    )
    loaded = RoutingSnapshot(path)
    try:
        result = SnapshotPlanner(loaded).get_ranked_route_result(
            ...
            [
                {
                    ...
                    "direction_id": 0,
                    "time_window": "morning_peak",
                    "reliability_probability": 0.91,
                    "sample_count": 100,
                }
            ],
            [],
            "route_direction_window",
            False,
        ),
        (
            [
...
def test_single_route_mode_stops_at_first_destination_and_does_less_work(tmp_path):
    ...
    try:
        ...
        assert (
            single.alternatives[0].itinerary == alternatives.alternatives[0].itinerary
        )
        ...
        assert (
            single.diagnostics.counters.states_popped
            < alternatives.diagnostics.counters.states_popped
        )
        assert (
            single.diagnostics.counters.labels_created
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"max_candidates": 1, "include_alternatives": True}, "maximum_candidates"),
    ],
)
...
def test_pickup_dropoff_transfer_and_maximum_transfer(tmp_path):
    ...
    try:
        ...
        origin_transfer = SnapshotPlanner(loaded).get_ranked_route_result(
            ...
            "C",
            date(2026, 8, 3),
            timedelta(seconds=29280),
            algorithm="dijkstra",
            max_transfers=0,
        )
        assert origin_transfer.alternatives[0].itinerary.arrival_time == timedelta(
            seconds=30000
        )
        ...
        assert destination_transfer.alternatives[0].itinerary.arrival_time == timedelta(
            seconds=29520
        )
    finally:
        loaded.close()
