...
from tests.routing.test_planner import MONDAY, FakeDatabase, at, connection, stops
...
def run(database, probabilities, **bounds):
    return TransitPlanner(database).get_ranked_route_result(
        ...
        "D",
        MONDAY,
        bounds.pop("departure", at(7, 59)),
        Resolver(probabilities),
        algorithm="mc_raptor",
        **bounds,
    )
...
def test_one_and_two_transfer_journeys_use_two_and_three_boardings():
    ...
    probabilities = {("R1", "B"): 0.9, ("R2", "C"): 0.9, ("R3", "D"): 0.9}
...
def test_reliability_is_once_per_boarded_leg_not_per_connection():
    ...
    assert result.alternatives[0].route_reliability == pytest.approx(0.75)
...
def test_two_75_percent_legs_multiply_to_5625_and_minimum_transfer_applies():
    trips = {
        "T1": [connection("T1", "weekday", "R1", "A", "B", at(8), at(8, 5))],
        "TOO_SOON": [
            connection("TOO_SOON", "weekday", "R2", "B", "D", at(8, 6), at(8, 15))
        ],
...
def test_slower_reliable_alternative_survives_and_dominated_trip_is_removed():
    trips = {
        ...
        "DOMINATED": [
            connection("DOMINATED", "weekday", "RX", "A", "D", at(8, 2), at(8, 20))
        ],
    }
    result = run(
        FakeDatabase(stops("A", "D"), trips),
        {("RF", "D"): 0.5, ("RS", "D"): 0.95, ("RX", "D"): 0.4},
    )
...
def test_service_date_filtering_is_preserved():
    trips = {"T": [connection("T", "weekend", "R", "A", "D", at(8), at(8, 10))]}
    ...
    assert not run(database, {("R", "D"): 0.9}).alternatives
...
def test_pattern_groups_exact_stop_sequences_and_deterministic_reconstruction():
    ...
    runs = [run(database, {("R", "D"): 0.9}) for _ in range(2)]
    assert trip_ids(runs[0]) == trip_ids(runs[1])
    assert [
        item.stop.stop_id for item in runs[0].alternatives[0].itinerary.legs[0].stops
    ] in (["A", "B", "D"], ["A", "C", "D"])
