...
def test_schema_uses_full_snapshot_identity_and_idempotent_insert():
    ...
    source = (root / "src" / "reliability" / "database.py").read_text(encoding="utf-8")
...
def test_profile_lookup_reads_materialized_profiles():
    ...
    assert "FROM latest" not in profile_source.split("    def route_profiles", 1)[0]
...
def test_exact_profiles_group_dates_into_shared_time_windows():
    ...
    source = (root / "src" / "reliability" / "database.py").read_text(encoding="utf-8")
    exact = source[
        source.index('        rebuild_exact = """') : source.index(
            '        count_latest = """'
        )
    ]
...
def test_profile_lookup_has_no_date_or_weekday_predicate():
    ...
    source = (root / "src" / "reliability" / "database.py").read_text(encoding="utf-8")
    lookup = source[
        source.index("    def profile(") : source.index("    def bulk_profile_data(")
    ]
...
def test_duplicate_snapshot_count_uses_batch_insert_result():
    ...
    inserted, duplicates = database.insert_observations([observation, observation])
